        ...
        def choose_move(query: QueryType) -> MoveType:
            match query:
                ...
                case QueryPlaceInitialTroop() as q:
                    ...
                    return handle_place_initial_troop_with_clusters_new(game, bot_state, q)
...
def handle_claim_territory_new(game: Game, bot_state: BotState, query: QueryClaimTerritory) -> MoveClaimTerritory:
    ...
    # Or if there are no such territories, we will pick just an unclaimed one with the greatest degree.
        ...
        #preferredStartingPoint = [31,40,8,32,25]
        preferredStartingPoint = [17,31,40,8,32]
        prefferedAvailable = list(set(preferredStartingPoint) & set(unclaimed_territories))
...
def handle_claim_territory(game: Game, bot_state: BotState, query: QueryClaimTerritory) -> MoveClaimTerritory:
    ...
    # Or if there are no such territories, we will pick just an unclaimed one with the greatest degree.
        ...
        preferredStartingPoint = [17,29,40,36,8,9]
        prefferedAvailable = list(set(preferredStartingPoint) & set(unclaimed_territories))
...
def handle_place_initial_troop_with_clusters_new(game: Game, bot_state: BotState, query: QueryPlaceInitialTroop) -> MovePlaceInitialTroop:
    my_territories = game.state.get_territories_owned_by(game.state.me.player_id)
    ...
    # print("[handle_place_initial_troop_with_clusters_new] --> Clustered territories ordered {}".format(cluster_list_ordered),flush=True)
    # print(f"[handle_place_initial_troop_with_clusters_new] - My territories: {my_territories}",flush=True)
    # print(f"[handle_place_initial_troop_with_clusters_new] - Non abandoned territories: {cluster_list_ordered[0]}",flush=True)
    # print(f"[handle_place_initial_troop_with_clusters_new] - Non abandoned Border territories: ",non_abandoned_border_territories,flush=True)
    # print(f"[handle_place_initial_troop_with_clusters_new] - Abandoned border territories: ",list(set(my_territories) - set(cluster_list_ordered[0])),flush=True)
    ...
    #print(f"[handle_place_initial_troop_with_clusters_new] - Border territory difference is: {border_territory_placement}",flush=True)

    #If there is a border territory with less than the max of its negbouring enemies, add troops there starting with the one with the biggest difference
    if len(border_territory_placement) > 0:
        print(f"[handle_place_initial_troop_with_clusters_new] -- Border territory placement: {border_territory_placement}")
        print(f"[handle_place_initial_troop_with_clusters_new] -- Placing a troop in {border_territory_placement[0][0]} because it has the greatest difference of {border_territory_placement[0][1]}")

        return game.move_place_initial_troop(query, border_territory_placement[0][0])
    else:
        #If all the territories have an equal number to the max of their enemy, place troops on a territory with a neighbour thats has most of its adjuscent territories surrounded by ours

        def close_to_surrounded_territory(territory):
            adjuscent = game.state.map.get_adjacent_to(territory)
            adjuscent_enemy = list(set(adjuscent) -  set(my_territories) )
            max_surroundex = (0,0,0) #Percentage,Friendly, Totaal
            for enemy in adjuscent_enemy:
                adjuscent_territories = game.state.map.get_adjacent_to(enemy)
                adjuscent_territories_mine = list(set(my_territories) & set(adjuscent_territories))
                percentage_surrounded = int((len(adjuscent_territories_mine)/len(adjuscent_territories))*100)

                if max_surroundex[0] < percentage_surrounded:
                    max_surroundex = (percentage_surrounded,len(adjuscent_territories_mine),len(adjuscent_territories))
            return max_surroundex
        
        #(territory_id, max_percentage_surrounded, no of troops)
        non_abandoned_border_territories_to_place = [(x,close_to_surrounded_territory(x),game.state.territories[x].troops) for x in non_abandoned_border_territories]
        non_abandoned_border_territories_to_place = sorted(non_abandoned_border_territories_to_place,key=lambda x: (x[1][0],x[2]),reverse=True)
        non_abandoned_border_territories_to_place_two = sorted(non_abandoned_border_territories_to_place,key=lambda x: (x[1][0]),reverse=True)

        print(f"[handle_place_initial_troop_with_clusters_new] -- My territories {my_territories}",flush=True)
        print(f"[handle_place_initial_troop_with_clusters_new] -- Non abandoned territories territories {cluster_list_ordered[0]}",flush=True)
        print(f"[handle_place_initial_troop_with_clusters_new] -- All territories have an equal number to their largest enemy neighbouring force. place troops on {non_abandoned_border_territories_to_place[0][0]} because it has a nearby enemy territory thats is {non_abandoned_border_territories_to_place[0][1]}% surrounded by our territories",flush=True)

        print(f"[handle_place_initial_troop_with_clusters_new] -- Non abandoned territories to place: {non_abandoned_border_territories_to_place}",flush=True)
        print(f"[handle_place_initial_troop_with_clusters_new] -- Non abandoned territories to place two: {non_abandoned_border_territories_to_place_two}",flush=True)
...
def handle_distribute_troops_new(game: Game, bot_state: BotState, query: QueryDistributeTroops) -> MoveDistributeTroops:
    ...
    #if len(game.state.recording) < 1500:
    #Start doom stacking when the number of redeemed cards is over 13
    if game.state.card_sets_redeemed > 13:
...
def handle_attack_with_probability_attack_weakest(game: Game, bot_state: BotState, query: QueryAttack) -> Union[MoveAttack, MoveAttackPass]:
    ...
    #We will decide on the territory to attack using the probability equation
    for attack_from in my_territory_sorted:
        print(f"[handle_attack_with_probability_attack_weakest] Attack from for loop {(time.perf_counter() - start)*1000} milli seconds")
        #Attack territory with mostly surrounded by my territories
        attack_from_adjuscent = game.state.map.get_adjacent_to(attack_from)
        ...
        for enemy in weakest_player:
            ...
            if len(adjuscent_enemy_player) > 0:
                print(f"[handle_attack_with_probability_attack_weakest] --> Thinking of attacking player {enemy} from {attack_from} which has {game.state.territories[attack_from].troops} troops and player enemies {adjuscent_enemy_player}",flush=True)
                ...
                #Find maximum amount of adjuscent enemies around except the one attacking
                    ...
                    if to_attack_enemy and len(game.state.recording) < start_attack_mode:
                        ...
                        print(f"[handle_attack_with_probability_attack_weakest]--[TEST]  -- Thinking of attacking {candidate} which has {game.state.territories[candidate].troops} troops. From {attack_from} which has {game.state.territories[attack_from].troops} troops Probability of attack success high ? {to_attack_enemy}. New possible enemies are: {potential_new_adjuscent_enemies_size}. Max: {max_new_possible_enemies}",flush=True)

                        #Will change this later to factor in the troops that will move
                        if (attack_from_troops - game.state.territories[candidate].troops) >= max_new_possible_enemies[1]:
                            print(f"[handle_attack_with_probability_attack_weakest]--[TEST]  --> Attacking from {attack_from} which has {attack_from_no_of_troops} to {candidate} which has {game.state.territories[candidate].troops} troops")
                            print(f"[handle_attack_with_probability_attack_weakest]--[TEST]  Before return {(time.perf_counter() - start)*1000} milli seconds")
                            #Attack with a minimum of 3 soldiers of the troops available
                            return game.move_attack(query,attack_from, candidate, min(3, attack_from_troops))
                        else:
...
def handle_attack_probability_simplifie_new(game: Game, bot_state: BotState, query: QueryAttack) -> Union[MoveAttack, MoveAttackPass]:
    ...
    #We will decide on the territory to attack using the probability equation
    for attack_from in my_territory_sorted:
        print(f"[handle_attack_probability_simplifie_new] Attack from for loop {(time.perf_counter() - start)*1000} milli seconds")
        #Attack territory with mostly surrounded by my territories
        attack_from_adjuscent = game.state.map.get_adjacent_to(attack_from)
        ...
        if game.state.territories[attack_from].troops >= 3 and len(attack_from_adjuscent_enemies) > 0:
            print("[handle_attack_probability_simplifie_new] --> Attack from territory has {} troops, and is surrounded by {} enemies".format(game.state.territories[attack_from].troops,len(attack_from_adjuscent_enemies)))
            ...
            print(f"[handle_attack_probability_simplifie_new] Before sorting candidate attack {(time.perf_counter() - start)*1000} milli seconds")
            candidate_enemy_attack = sorted(attack_from_adjuscent_enemies,key=territory_most_surrounded)
            print(f"[handle_attack_probability_simplifie_new] After sorting candidate attack {(time.perf_counter() - start)*1000} milli seconds")

            candidate_enemy_attack_and_troops = [(x,game.state.territories[x].troops) for x in candidate_enemy_attack]


            print(f"[handle_attack_probability_simplifie_new] --> [NEW-TEST] Territory {attack_from} troops {attack_from_no_of_troops}. Possible candidates to attack and troops: {candidate_enemy_attack_and_troops}",flush=True)


            #Get territories where the attack probability is greater than probability threshold 
        

            #Attack first enemy with an attack probability greater than the threshold, and where you will have more troops than your new adjuscent neighbours
            for candidate in candidate_enemy_attack:
                #Only atack if you have 3 more than the adjuscent enemy except the one you are attacking
                #Find maximum amount of adjuscent enemies around except the one attacking
                to_attack_enemy = to_attack(game,attack_from_troops,game.state.territories[candidate].troops)
                ...
                if to_attack_enemy:
                    ...
                    print(f"[handle_attack_probability_simplifie_new]--[TEST]  -- Thinking of attacking {candidate} which has {game.state.territories[candidate].troops} troops. From {attack_from} which has {game.state.territories[attack_from].troops} troops Probability of attack success high ? {to_attack_enemy}. New possible enemies are: {potential_new_adjuscent_enemies_size}. Max: {max_new_possible_enemies}",flush=True)


                    
                    current_attack_from_adjuscent_enemies = list(set(attack_from_adjuscent) - set(my_territories))
                    print(f"[handle_attack_probability_simplifie_new]--[TEST] Current attack adjuscent enemies: {current_attack_from_adjuscent_enemies}",flush=True)
                    current_attack_from_adjuscent_enemies.remove(candidate)
                    print(f"[handle_attack_probability_simplifie_new]--[TEST] After remove Current attack adjuscent enemies: {current_attack_from_adjuscent_enemies}",flush=True)

                    if len(current_attack_from_adjuscent_enemies) > 1:
                        current_max_enemy = max(current_attack_from_adjuscent_enemies,key=lambda x: game.state.territories[x].troops)
                    else:
                        current_max_enemy = 0

                    print(f"[handle_attack_probability_simplifie_new]--[TEST]  --> Current max adjuscent {current_max_enemy}")

                    #Will change this later to factor in the troops that will move
                    if (attack_from_troops - game.state.territories[candidate].troops) >= max_new_possible_enemies[1]:
                        print(f"[handle_attack_probability_simplifie_new]--[TEST]  --> Attacking from {attack_from} which has {attack_from_no_of_troops} to {candidate} which has {game.state.territories[candidate].troops} troops")
                        print(f"[handle_attack_probability_simplifie_new]--[TEST]  Before return {(time.perf_counter() - start)*1000} milli seconds")
                        #Attack with a minimum of 3 soldiers of the troops available
                        return game.move_attack(query,attack_from, candidate, min(3, attack_from_troops))
                    else:
                        print(f"[handle_attack_probability_simplifie_new]--[TEST] --> Not attacking  from {attack_from} which has {attack_from_no_of_troops} to {candidate} which has {game.state.territories[candidate].troops} troops. Because I will have new adjuscent enemies {potential_new_adjuscent_enemies_size} wtih a max of {max_new_possible_enemies}")
                    ...
                    print(f"[handle_attack_probability_simplifie_new] --> Not Attacking from {attack_from} which has {attack_from_no_of_troops} to {candidate} which has {game.state.territories[candidate].troops} troops because probability of success and below threshold")

        else:
            print(f"[handle_attack_probability_simplifie_new] --> Passing attack from {attack_from}. Reason troops: {game.state.territories[attack_from].troops} or enemies: {attack_from_adjuscent_enemies}")
            pass
            
            
    print(f"[handle_attack_probability_simplifie_new] [CHECK] --> There is no adjuscent territory available for attack at all. Proceeding without attack")
    return game.move_attack_pass(query)
